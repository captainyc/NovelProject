...
meta_ch = read.csv("./test/CmetricsFinal.csv")
meta_jp = read.csv("./test/JmetricsFinal.csv")
feature_name = c("ttr_mean","ttr_sd","ttr_norm","ttr_cumsum",
                 ...
                 "thought2","pronoun_first","pronoun_third",
...
for (i in 1:nrow(meta_ch)) {
	text = scan(paste0("~/github/NovelProject/data/chinese/",meta_ch$file_text_name[i]), what="character", sep=" ", quote="", quiet=TRUE)
	text = text[text!=""]
	...
	meta_ch[i,"thought2"] = mean(text %in% thought$ch)
...
for (i in 1:nrow(meta_ch)) {
	...
	record_ch = rbind(record_ch,data.frame(file_id=rep(meta_ch$file_text_name[i],length(ttr)),chunk_no=1:length(ttr),ttr=ttr,ent=ent,con_ent=con_ent))
	setTxtProgressBar(pb,i/nrow(meta_ch))
}
close(pb)
...
for (i in 1:nrow(meta_jp)) {
	...
	meta_jp[i,"thought2"] = mean(text %in% thought$jp)
